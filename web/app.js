...
const STATIC_I18N = {
  en: {
    ...
    hintTodoWalls: 'The front lip sits on the screen side — keep it short so it barely covers the display. The back lip is hidden behind the display and forms the ESP32 pocket — make it tall enough to cover the ESP32 (24mm).',
    secTodoComp: 'Electronics (ESP32)', lblTEsp: 'ESP32 back pocket',
    hintTodoComp: 'The ESP32-C3 supermini drops into a pocket in the back wall (hidden behind the display), USB facing down into a flared USB-C port in the base so the cable plugs in from below.',
    secTodoExport: 'STL export', btnTEx1: 'Todo supporter.stl',
    hintTodoExport: 'One printed part, a slim column the width of the ESP32. Print it as it appears (bridge on the plate, clip opening facing up) — the chamfered top and pocket need no supports.',
  },
  ko: {
    ...
    hintTodoWalls: '앞턱은 화면 쪽에 위치합니다(화면을 거의 가리지 않게 짧게 유지). 뒤턱은 화면 뒤에 숨어 ESP32 포켓을 이룹니다 — ESP32(24mm)를 덮을 만큼 충분히 높게 하세요.',
    secTodoComp: '전자부품 (ESP32)', lblTEsp: 'ESP32 뒷면 포켓',
    hintTodoComp: 'ESP32-C3 슈퍼미니가 뒷벽 포켓에 꽂히고(화면 뒤에 숨김), USB는 아래를 향해 바닥의 나팔형 USB-C 포트로 케이블을 아래에서 꽂습니다.',
    secTodoExport: 'STL 내보내기', btnTEx1: '투두 서포터.stl',
    hintTodoExport: 'ESP32 폭만큼의 슬림한 기둥 한 개짜리 출력 파트입니다. 보이는 그대로 출력하세요(브릿지를 바닥에, 클립 입구가 위로) — 챔퍼된 상단과 포켓 모두 서포트가 필요 없습니다.',
  },
};
